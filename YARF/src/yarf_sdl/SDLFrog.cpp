...
SDLFrog::SDLFrog(SDL_Renderer * Renderer, SDL_Texture * Texture,
		...
		double &width_scale_factor, double &height_scale_factor, list<yarf::Bullet *> &bullet_list)
{
	...
	frogRotatePoint.y = frogDestRect.h / 3; 	// y-point moved closer to the head of the frog
}
...
void SDLFrog::Vis(int direction, int leap, const int & FPS)
{
	...
	else
	{
		...
		SDL_RenderCopyEx(m_pRenderer, m_pTexture, &frogSrcRect,
				&frogDestRect, position.theta, &frogRotatePoint,
				SDL_FLIP_NONE);
	}
}
...
{
	frogDestRect.w = position.w * 160 * (*m_widthScaleFactor);
	frogDestRect.h = position.h * 160 * (*m_heightScaleFactor);
	frogDestRect.x = position.x * 160 * (*m_widthScaleFactor);
	frogDestRect.y = position.y * 160 * (*m_heightScaleFactor);
}
...
void SDLFrog::LeapVis(int & direction) //visualising the leaping frog, rather than default frog
{
	...
	SDL_RenderCopyEx(m_pRenderer, m_pTexture, &frogSrcRect, &frogDestRect,
			position.theta, &frogRotatePoint, SDL_FLIP_NONE);
	frogSrcRect.x = 0;
}
...
void SDLFrog::CreateBullet()
{
	...
	m_bulletList->push_back(
			new yarf_sdl::SDLBullet(m_pRenderer, m_pTexture, pWindowRect,
					windowWidth, windowHeight, *m_widthScaleFactor, *m_heightScaleFactor,
